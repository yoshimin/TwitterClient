...
class HomeTimelineViewModel {
    ...
    /// タイムラインプロバイダー
    private let timelineProvider = Variable<TimelineProvider<HomeTimelineLoader>?>(nil)
    ...
    init() {
        ...
        let account = AccountProvider.shared.account
            .asObservable()
        ...
        account
            .map({ account -> TimelineProvider<HomeTimelineLoader>? in
                guard let account = account else {
                    return nil
                }
                let loader = HomeTimelineLoader(account: account)
